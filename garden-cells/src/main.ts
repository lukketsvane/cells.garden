import { App, ItemView, Modal, Plugin, Setting, WorkspaceLeaf, parseYaml, TFile, TFolder } from 'obsidian';
import Sortable, { SortableEvent } from 'sortablejs';

// --- ASSET IMPORTS ---
// @ts-ignore
import bgImageUrl from './images/bg_image.png';
// @ts-ignore
import cloudUrl from './images/cloud.png';
// @ts-ignore
import mountainsUrl from './images/mountains.png';
// @ts-ignore
import groundUrl from './images/ground_tile.png';
// @ts-ignore
import flower1Url from './images/plant_1/plant_1_flower1.png';
// @ts-ignore
import flower2Url from './images/plant_1/plant_1_flower2.png';

// @ts-ignore
import ant1Url from './images/ant_walk_1.png';
// @ts-ignore
import ant2Url from './images/ant_walk_2.png';

// @ts-ignore
import stem1Url from './images/plant_1/plant_1_part1.png';
// @ts-ignore
import stem2Url from './images/plant_1/plant_1_part2.png';
// @ts-ignore
import stem3Url from './images/plant_1/plant_1_part3.png';
// @ts-ignore
import stem4Url from './images/plant_1/plant_1_part4.png';
// @ts-ignore
import stem5Url from './images/plant_1/plant_1_part5.png';
// @ts-ignore
import stem6Url from './images/plant_1/plant_1_part6.png';
// @ts-ignore
import stem7Url from './images/plant_1/plant_1_part7.png';
// @ts-ignore
import stem8Url from './images/plant_1/plant_1_part8.png';
...
const PLANT_TYPES = ['plant_1', 'plant_2', 'plant_3', 'plant_4', 'plant_5', 'plant_6', 'plant_7', 'plant_8'];
...
interface ProjectData {
    ...
    standby: boolean; // <--- Changed from lightOff
    hue: number;
    order: number;
    plantType: string;
    roots: LayerItem[];
...
interface GardenPluginSettings {
...
// --- THESE COLORS AREN'T IN USE. Theyre based on code I didn't implement for using hex and converting to something the math can use ---
...
// --- 2. Constants ---
const GARDEN_VIEW_TYPE = "garden-view";

const PIXEL_SCALE = 4; 
const PLANT_SPACING = 300; 
const GROUND_HEIGHT_PERCENT = 35;
const CLOUD_SCROLL_DURATION = 500; // seconds for one full loop
const STEM_ORIGIN_WIDTH = 35;
const STEM_ORIGIN_HEIGHT = 7;
const STEM_OVERLAP_ORIGIN = 1; 
const FIXED_STACK_STEP = (STEM_ORIGIN_HEIGHT - STEM_OVERLAP_ORIGIN) * PIXEL_SCALE; // 6 * 4 = 24px
...
class SetupModal extends Modal {
    ...
    constructor(app: App, plugin: GardenPlugin, onSubmit: (path: string) => void) {
        super(app);
        this.plugin = plugin;
    ...
    onOpen() {
        ...
        contentEl.createEl("h2", { text: "🌱 Garden Cells Setup" });
        contentEl.createEl("p", { text: "Please enter a name for your Garden folder. (If it exists, it will be used. Otherwise, it will be created in your vault root.)" });

        let folderName = "Garden-Cells";

        new Setting(contentEl)
            .setName("Folder Name")
            .addText((text) => {
                text.setValue(folderName);
                text.onChange((value) => { folderName = value; });
        ...
        new Setting(contentEl)
            .addButton((btn) => {
                btn.setButtonText("Start Garden")
                    .setCta()
                    .onClick(() => {
                        if (folderName.trim()) {
                            let path = folderName.trim();
...
// --- 4. The Confirm Delete Modal ---

class ConfirmDeleteModal extends Modal {
    onSubmit: () => void;
    plantName: string;

    constructor(app: App, plantName: string, onSubmit: () => void) {
        super(app);
        this.onSubmit = onSubmit;
...
// --- 5. The Create Project Modal ---

class CreateProjectModal extends Modal {
    onSubmit: (seed: string) => void;

    constructor(app: App, onSubmit: (seed: string) => void) {
        super(app);
        this.onSubmit = onSubmit;
    }
...
class AddItemModal extends Modal {
    ...
    constructor(app: App, title: string, placeholder: string, onSubmit: (content: string) => void) {
        super(app);
        this.title = title;
...
class AssetManager {
    ...
    private getPathsInFolder(folderPath: string): string[] {
        const paths: string[] = [];
        // Obsidian's getAbstractFileByPath fails if there is a trailing slash!
        ...
        if (folder instanceof TFolder) {
            ...
            for (const file of folder.children) {
                if (file instanceof TFile && validExtensions.includes(file.extension.toLowerCase())) {
                    paths.push(file.path);
                }
            }
            console.log(`Garden Cells: Found ${paths.length} images in ${cleanPath}`);
        } else {
            console.warn(`Garden Cells: Asset folder NOT found at "${cleanPath}".`);
        }
        return paths;
    }
    ...
    assignRandomImage(category: string, plantType?: string): string | null {
        ...
        if (plantType) {
            paths = this.getPathsInFolder(`Garden-Assets/${plantType}/${category}/`);
        }
        
        // 2. Fallback to the shared category folder (used for roots, minerals, seeds, or if plant folder is empty)
        if (paths.length === 0) {
            paths = this.getPathsInFolder(`Garden-Assets/${category}/`);
        }
...
class GardenView extends ItemView {
    ...
    private saveViewState() {
        ...
        this.plugin.settings.viewState = state;
    ...
    private loadViewState(): ViewState | null {
        return this.plugin.settings.viewState ?? null;
    }
    ...
    private _starsGifDataUrl: string | null = null;
    ...
    constructor(leaf: WorkspaceLeaf, plugin: GardenPlugin) {
        super(leaf);
        this.plugin = plugin;
    }

    getViewType() { return GARDEN_VIEW_TYPE; }
    getDisplayText() { return "Project Garden"; }

    _skipNextModify = false;
    ...
    async onOpen() {
        ...
        try {
            ...
            // If data hasn't loaded yet (e.g. view restored before onLayoutReady), load it now
            if (this.plugin.gardenData.length === 0 && this.plugin.settings.dataFolderPath) {
                await this.plugin.loadGardenData();
            }
                    ...
                    win.requestAnimationFrame(() => {
                        win.requestAnimationFrame(() => {
                            ...
                            if (viewport && world) {
                                ...
                                const plantCount = this.plugin.gardenData.length;
    ...
    async renderGarden(container: HTMLElement) {
        ...
        if (this.plugin.gardenData.length === 0) {
            ...
            this.plugin.gardenData.forEach(project => {
    ...
    private async renderGardenCanvas(parent: HTMLElement) {
        ...
        const calculatedWidth = Math.max(600, this.plugin.gardenData.length * PLANT_SPACING + WORLD_PADDING * 2);
        world.style.width = `${calculatedWidth}px`;

        // --- Phase 1: Preload all images first ---
        for (const project of this.plugin.gardenData) {
        ...
        const starsUrl = await this.loadStarsGif();
        if (starsUrl) {
    ...
    private async preloadImages(project: ProjectData) {
        ...
        const promises = pathsToLoad.map(path => this.plugin.assetManager.getImageUrl(path));
    ...
    private async loadStarsGif(): Promise<string | null> {
        if (this._starsGifDataUrl) return this._starsGifDataUrl;
        try {
            const pluginDir = this.plugin.manifest.dir;
            const filePath = `${pluginDir}/src/images/stars_pattern.gif`;
            const buffer = await this.app.vault.adapter.readBinary(filePath);
            const blob = new Blob([buffer], { type: 'image/gif' });
            this._starsGifDataUrl = URL.createObjectURL(blob);
            return this._starsGifDataUrl;
        } catch (e) {
            console.warn('Garden Cells: Could not load stars_pattern.gif', e);
            return null;
        }
    }
    ...
    private async renderPlantSprite(parent: HTMLElement, project: ProjectData) {
        ...
        for (const block of aboveStack) {
            ...
            if (block.imagePath) {
                url = this.plugin.assetManager.getImageUrlSync(block.imagePath);
            }
    ...
    private async deleteSelectedCells() {
        ...
        for (const sel of toDelete) {
            const project = this.plugin.gardenData.find(p => p.id === sel.projectId);
        ...
        this.clearSelection();
        this._skipNextModify = true;
        await this.plugin.saveGardenData();
        this.onOpen();
    }
...
private _splitRatio = 0.5; // persisted divider position (0 = top, 1 = bottom)
    ...
    private showSeedContextMenu(e: MouseEvent, project: ProjectData, seedContent: HTMLElement) {
        ...
        // 1. Open File
        const openOpt = document.createElement('button');
        openOpt.textContent = 'Open File';
        openOpt.style.cssText = menuStyle;
        openOpt.onmouseenter = () => openOpt.style.background = hoverStyle;
        openOpt.onmouseleave = () => openOpt.style.background = 'none';
        openOpt.onclick = async (ev) => {
            ev.stopPropagation();
            menu.remove();
            const file = await this.plugin.getProjectFile(project.id);
            if (file) this.app.workspace.openLinkText(file.path, '');
        };
        ...
        deleteOpt.onclick = (ev) => {
            ev.stopPropagation();
            menu.remove();
            new ConfirmDeleteModal(this.app, project.seed, async () => {
                const index = this.plugin.gardenData.findIndex(p => p.id === project.id);
                if (index !== -1) {
                    this.plugin.gardenData.splice(index, 1);
                    this._skipNextModify = true;
                    await this.plugin.saveGardenData();
                    this.onOpen();
                }
        ...
        PLANT_TYPES.forEach(pt => {
            if (pt !== project.plantType) {
                ...
                typeOpt.onclick = async (ev: MouseEvent) => {
                    ...
                    project.stem.forEach(item => { item.imagePath = this.plugin.assetManager.assignRandomImage('stem', pt) || undefined; });
                    project.flowers.forEach(item => { item.imagePath = this.plugin.assetManager.assignRandomImage('flowers', pt) || undefined; });
...
const seedContent = seedCell.createDiv({ text: project.seed, cls: "seed-content draggable-cell", attr: { tabindex: "0" } });
        ...
        seedContent.addEventListener("blur", () => {
            if (seedContent.hasClass("is-editing")) {
                ...
                if (newSeed && newSeed !== project.seed) {
                    ...
                    this.plugin.saveGardenData();
                    this.plugin.renameProjectFile(project);
                } else if (!newSeed) {
            ...
            el.addEventListener("contextmenu", (e: MouseEvent) => {
                ...
                highlightOpt.onclick = async () => {
                    ...
                    for (const cell of cellsToHighlight) {
                        ...
                        const cItemId = cell.dataset.id;
                        const cProject = this.plugin.gardenData.find(p => p.id === cProjectId);
                        ...
                        for (const cell of cellsToConvert) {
                            ...
                            if (cProject) {
                                ...
                                if (index !== -1) {
                                    ...
                                    cItem.imagePath = this.plugin.assetManager.assignRandomImage('stem', cProject.plantType) || undefined;
    ...
    async handleDrop(evt: SortableEvent) {
        ...
        const sourceProject = this.plugin.gardenData.find(p => p.id === sourceProjectId);
        const targetProject = this.plugin.gardenData.find(p => p.id === targetProjectId);
        ...
        if (sourceArrayName !== targetArrayName) {
            movedItem.imagePath = this.plugin.assetManager.assignRandomImage(targetArrayName, targetProject.plantType) || undefined;
        }
    ...
    async handleColumnDrop(evt: SortableEvent) {
        ...
        const maxIdx = this.plugin.gardenData.length - 1;
        if (oldIdx < 0 || newIdx < 0 || oldIdx > maxIdx || newIdx > maxIdx) return;

        const [movedProject] = this.plugin.gardenData.splice(oldIdx, 1);
        if (!movedProject) return;

        // Adjust newIdx if we removed an item before it
        if (oldIdx < newIdx) newIdx--;
        this.plugin.gardenData.splice(newIdx, 0, movedProject);

        // UPDATE ALL ORDERS: Assign 0, 1, 2, 3... based on current array position
        this.plugin.gardenData.forEach((proj, idx) => {
            proj.order = idx;
        });
     ...
     async createNewProject(position: 'left' | 'right' = 'right') {
        new CreateProjectModal(this.app, async (seed) => {
            const seedImagePath = this.plugin.assetManager.assignRandomImage('seeds'); 
             ...
             const newProject: ProjectData = {
                ...
                order: this.plugin.gardenData.length,
            ...
            if (position === 'left') {
                this.plugin.gardenData.unshift(newProject);
            } else {
                this.plugin.gardenData.push(newProject);
            }
    ...
    async addNewItem(project: ProjectData, arrayName: 'flowers' | 'minerals' | 'roots' | 'stem') {
        ...
        new AddItemModal(this.app, title, placeholder, async (content) => {

            // Ask the manager for a random image from the matching vault folder
            const randomImagePath = this.plugin.assetManager.assignRandomImage(arrayName, project.plantType);
...
export default class GardenPlugin extends Plugin {
    ...
    settings: GardenPluginSettings = DEFAULT_SETTINGS;
    private modifyTimeout = 0;
    assetManager!: AssetManager; 
    private _needsImageSave: boolean = false;
    ...
    async onload() {
        ...
        // Register the view type IMMEDIATELY so the workspace can restore it on reload
        this.registerView(GARDEN_VIEW_TYPE, (leaf) => new GardenView(leaf, this));
        this.addRibbonIcon('flower-2', 'Open Garden', () => this.activateView());
        this.addCommand({
            id: 'open-garden',
            name: 'Open Garden View',
            callback: () => this.activateView()
        });

        if (!this.settings.dataFolderPath) {
            const likelyFolder = this.app.vault.getFolderByPath("Garden-Cells".replace(/\/$/, ""));
            if (likelyFolder) {
                console.log("Garden Cells: Automatically recovered Garden-Cells/");
                this.settings.dataFolderPath = "Garden-Cells/";
                this.saveSettings().catch(() => {});
            } else {
                this.app.workspace.onLayoutReady(() => this.openSetupModal());
                return; 
            }
        }

        this.app.workspace.onLayoutReady(() => {
            this.initializeViewAndData();
        });
    }

    onunload() {}

    async initializeViewAndData() {
        await this.loadGardenData();

        // If we auto-assigned missing images during load, save them back to the files!
        if (this._needsImageSave) {
            this._needsImageSave = false;
            await this.saveGardenData();
        }

        // Refresh any Garden views that were already restored by the workspace (with empty data)
        this.app.workspace.getLeavesOfType(GARDEN_VIEW_TYPE).forEach(leaf => {
        ...
        this.registerEvent(this.app.vault.on('modify', (file) => {
            if (file.path.startsWith(this.settings.dataFolderPath)) {
                // Skip if this modify was triggered by our own save (within 2s of last save)
                const view = this.app.workspace.getLeavesOfType(GARDEN_VIEW_TYPE)
                    .find(l => l.view instanceof GardenView)?.view as GardenView | undefined;
                if (view?._skipNextModify || (Date.now() - (view?._lastSaveTimestamp || 0)) < 2000) {
                    view && (view._skipNextModify = false);
                    return;
                }

                clearTimeout(this.modifyTimeout);
                this.modifyTimeout = window.setTimeout(async () => {
                    this.app.metadataCache.trigger('changed', file);
                    await this.loadGardenData(file as TFile); 
                    this.app.workspace.getLeavesOfType(GARDEN_VIEW_TYPE).forEach(leaf => {
                        if (leaf.view instanceof GardenView) {
                            leaf.view.onOpen();
                        }
                ...
                // LISTEN FOR EXTERNAL RENAMES (e.g., user renames file in the sidebar)
        this.registerEvent(this.app.vault.on('rename', (file, oldPath) => {
            if (file.path.startsWith(this.settings.dataFolderPath) && file instanceof TFile) {
                // Safely extract frontmatter to a definite variable so TS 4.7 stops complaining
                const cache = this.app.metadataCache.getFileCache(file);
                const fm = cache ? cache.frontmatter : undefined;
                
                if (fm && fm.id) {
                    const proj = this.gardenData.find(p => p.id === fm.id);
                    if (proj && proj.seed !== file.basename) {
                        // Update the memory data to match the new file name
                        proj.seed = file.basename;
                        proj.name = file.basename;
                        
                        // Refresh the UI so the column updates
                        this.app.workspace.getLeavesOfType(GARDEN_VIEW_TYPE).forEach(leaf => {
                            if (leaf.view instanceof GardenView) {
                                const view = leaf.view as GardenView;
                                const scrollStates = view.saveScrollPositions();
                                view.onOpen();
                                view.restoreScrollPositions(scrollStates);
                            }
                        });
                    }
                }
    ...
    async openSetupModal() {
        new SetupModal(this.app, this, async (path) => {
            if (!path.endsWith('/')) path += '/';
            
            let folder = this.app.vault.getFolderByPath(path.replace(/\/$/, ""));
            if (!folder) {
                try {
                    await this.app.vault.createFolder(path);
                } catch (e) {
                    folder = this.app.vault.getFolderByPath(path);
                }
            }

            this.settings.dataFolderPath = path;
            await this.saveSettings();

            // initializeViewAndData was skipped (early return), so load data and set up events now
            await this.initializeViewAndData();
    ...
    async loadSettings() {
        ...
        if (raw.dataFilePath && !raw.dataFolderPath) {
            console.log("Garden Cells: Migrating old file setting to folder setting.");
            raw.dataFolderPath = raw.dataFilePath;
            delete raw.dataFilePath;
    ...
    async loadGardenData(forceReadFile?: TFile) {
        ...
        for (const child of allFiles) {
            if (!child.path.startsWith(folderPath)) continue;

            try {
                let fm = this.app.metadataCache.getFileCache(child)?.frontmatter;

                // If this is the file the user just modified, bypass the stale cache!
                if (forceReadFile && child.path === forceReadFile.path) {
                    fm = undefined; 
                }

                // If cache has no frontmatter (or we bypassed it), try reading directly
                if (!fm || !fm.type) {
                    ...
                    try {
                        ...
                        console.error("Garden Cells: Direct read failed", child.path, e);
                    }
                }

                if (!fm || fm.type !== 'garden-cell') continue;
                ...
                if (isNewFormat) {
                    try {
                        ...
                        const parseListItems = (sectionName: string, category: string): LayerItem[] => {
                            ...
                            return sectionBody.split('\n')
                                ...
                                .map((line, idx) => {
                                    ...
                                    let stripped = line.replace(/^- \[[ x]\] /, '').replace(/^- /, '');
                                    // Check for **highlighted** syntax
                                    const isHighlighted = /^\*\*(.+)\*\*$/.test(stripped);
                                    const content = isHighlighted ? stripped.replace(/^\*\*(.+)\*\*$/, '$1') : stripped;
                                    
                                    // Explicitly type as string | undefined to satisfy TypeScript
                                    let imgPath: string | undefined = imgPaths[idx];

                                    // AUTO-ASSIGN: If the text was added manually and has no image, grab one!
                                    if (!imgPath) {
                                        imgPath = this.assetManager.assignRandomImage(category, fm?.plantType) || undefined;
                                        if (imgPath) this._needsImageSave = true; // Mark that we need to save this to the file
                                    }
                        ...
                        console.error("Garden Cells: New format parse failed for", child.path, e);
                    }
                } else {
                    // Old format: items in frontmatter YAML arrays
                    const parseArr = function(arr: any[]): LayerItem[] {
                        if (!arr) return [];
                ...
                this.gardenData.push({
                    id: projectId,
                    name: child.basename, 
                    seed: fm.seed || child.basename,
                    seedImagePath: fm.seedImagePath || undefined,
                    standby: fm.standby === true, // <--- Changed
                    hue: fm.hue || 0,
                    order: fm.order || 0,
                    plantType: fm.plantType || 'plant_1', // <--- ADD THIS
                    roots,
                    stem,
                    flowers,
                    minerals
                ...
                console.error("Garden Cells: Failed to load file", child.path, e);
            }
        }
        ...
        try {
            // Mark the save time so the modify event handler can skip our own changes
            const view = this.app.workspace.getLeavesOfType(GARDEN_VIEW_TYPE)
                .find(l => l.view instanceof GardenView)?.view as GardenView | undefined;
            if (view) view._lastSaveTimestamp = Date.now();

            console.log(">>> SAVE CALLED. Projects in memory:", this.gardenData.length);
            const folderPath = this.settings.dataFolderPath;
            
            if (!folderPath) {
                console.log(">>> ABORTED: No folder path set.");
                return;
            }


            
            let folder = this.app.vault.getFolderByPath(folderPath.replace(/\/$/, "")); // STRIP SLASH FOR LOOKUP
            
            if (!folder) {
                try {
                    folder = await this.app.vault.createFolder(folderPath);
                } catch (e) {
                    folder = this.app.vault.getFolderByPath(folderPath);
                }
            }

            if (!folder) {
                console.log(">>> ABORTED: Folder does not exist and couldn't be created.");
                return;
            }

            const updatedIds = new Set<string>();

            for (const project of this.gardenData) {
                console.log(">>> LOOP START: Processing", project.id, project.seed);
                
                updatedIds.add(project.id);
                const frontmatter = this.stringifyProjectFrontmatter(project);
                const body = this.stringifyProjectBody(project);
                const fileContent = `---\n${frontmatter}---\n${body ? '\n' + body : ''}`;
                console.log(">>> FILE CONTENT GENERATED for", project.id);
                
                let existingFile = await this.getProjectFile(project.id);
                console.log(`>>> FILE LOOKUP RESULT for ${project.id}:`, existingFile ? existingFile.path : "NULL");
                ...
                if (existingFile) {
                    ...
                    console.log(">>> FILE NOT FOUND. ATTEMPTING TO CREATE NEW FILE...");
                    const safeName = project.seed.replace(/[^a-z0-9 _-]/gi, '').trim() || "Untitled Plant";
                    let filePath = `${folderPath}/${safeName}.md`;
                    
                    try {
                        await this.app.vault.create(filePath, fileContent);
                        console.log(">>> SUCCESSFULLY CREATED NEW FILE!");
                    } catch (e) {
                        console.error(">>> CREATE FAILED, TRYING FALLBACK:", e);
                        const fallbackFile = this.app.vault.getFileByPath(filePath);
                        if (fallbackFile instanceof TFile) {
                            await this.app.vault.modify(fallbackFile, fileContent);
            ...
            console.log(">>> LOOP FINISHED. DELETING ORPHANS...");
            const allFiles = this.app.vault.getMarkdownFiles();
            for (const child of allFiles) {
                if (!child.path.startsWith(folderPath)) continue;
                if (child instanceof TFile && child.extension === 'md') {
                    const cache = this.app.metadataCache.getFileCache(child);
                    if (cache?.frontmatter?.type === 'garden-cell' && !updatedIds.has(cache.frontmatter.id)) {
            ...
            console.error(">>> CATASTROPHIC ERROR IN saveGardenData:", mainError);
        }
    }

    async renameProjectFile(project: ProjectData) {
        const file = await this.getProjectFile(project.id);
        if (!file) return;
        
        const folderPath = this.settings.dataFolderPath;
        const safeName = project.seed.replace(/[^a-z0-9 _-]/gi, '').trim() || "Untitled Plant";
        let newPath = `${folderPath}/${safeName}.md`;
        
        // If the calculated path is exactly the same, do nothing
        if (file.path === newPath) return;
        
        try {
            await this.app.vault.rename(file, newPath);
        } catch (e) {
            console.error("Garden Cells: Failed to rename file", e);
        }
    }

    async getProjectFile(projectId: string): Promise<TFile | null> {
        const folderPath = this.settings.dataFolderPath;
        ...
        for (const child of allFiles) {
            ...
            const cache = this.app.metadataCache.getFileCache(child);
            if (cache?.frontmatter?.id === projectId) return child;

            // 2. CACHE FALLBACK: The cache is asleep, read the file directly from the hard drive!
            try {
                const contents = await this.app.vault.read(child);
                const fmMatch = contents.match(/^---\r?\n([\s\S]*?)\r?\n---/);
                if (fmMatch) {
                    const fm = parseYaml(fmMatch[1]);
                    if (fm?.id === projectId) return child;
                }
            } catch (e: unknown) {
                console.error("Garden Cells: Direct read failed", child.path, e);
            }
    ...
    private stringifyProjectFrontmatter(p: ProjectData): string {
    ...
    private stringifyProjectBody(p: ProjectData): string {
        const lines: string[] = [];
