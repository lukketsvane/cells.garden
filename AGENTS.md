...
- **Max's plugin.** `obsidian/` is the original Obsidian plugin and is not edited.
- **Checks before pushing.** `npm run typecheck`, `npm run build`, `npm run test:web`; also `npm run build:ext` and `npm run test:ext` when touching `ext/`.
